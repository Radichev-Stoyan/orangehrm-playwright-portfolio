...
test.describe('Admin - User management', () => {
    ...
    test('Should show required validation when User Role is empty', async ({ page }, testInfo) => {
        const sideMenu = new SideMenu(page, testInfo.project.name);
    ...
    test('Should show required when Status is empty', async ({ page }, testInfo) => {
        const sideMenu = new SideMenu(page, testInfo.project.name);
        ...
        await adminPage.createUserAdminPage(employee.firstName, { skipStatus: true });

        const statusField = page
            .locator('.oxd-input-group')
            .filter({ hasText: 'Status' });

        await expect(statusField.getByText('Required')).toBeVisible();
    })
